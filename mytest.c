		...
		switch(choice){
			case '0': printf("\n[CHOICE:INPUT] Number of jobs : ");
				  ...
				  for(i=0;i<size;i++)
					  addJobToPool(pool, &longJob, NULL);
				  break;
			case '1': printf("\n[CHOICE:INFO] Pending jobs %" PRIu64 , getJobCount(pool));
				  break;
			case '2': addThreadsToPool(pool, 1);
				  break;
			case '3': removeThreadFromPool(pool);
				  break;
			case '4': printf("\n[CHOICE:INFO] Number of threads %" PRIu64, getThreadCount(pool));
				  break;
			case '5': waitToComplete(pool);
				  break;
			case '6': suspendPool(pool);
				  break;
			case '7': resumePool(pool);
				  break;
